...
class CMapAPI(object):
	...
	def _generate_request(self, query_args):
		...
		'''
		req = {}
		for key, thisarg in list(query_args.items()):
	...
	def _check_fields(self, res, fields):
		...
		'''
		allfields = sorted(set([z for y in [list(x.keys()) for x in res if x] 
		                   		for z in y]))
		...
		if self.verbose:
			indiv_missing = [np.setdiff1d(allfields, list(x.keys()))
							 for x in res if x]
